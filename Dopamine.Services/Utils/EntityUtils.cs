...
namespace Dopamine.Services.Utils
{
    public static class EntityUtils
    {
        ...
        public static bool FilterSongs(SongViewModel album, string filter)
        {
            // Trim is required here, otherwise the filter might flip on the space at the beginning (and probably at the end)
            string[] pieces = filter.Trim().Split(Convert.ToChar(" "));

            return pieces.All((s) =>
            album.SongTitle.ToLower().Contains(s.ToLower()) |
            album.AlbumArtist.ToLower().Contains(s.ToLower()) |
            album.Year.ToString().ToLower().Contains(s.ToLower()));
        }
        ...
        public static bool FilterFolder(FolderViewModel folder, string filter)
        {
            // Trim is required here, otherwise the filter might flip on the space at the beginning (and probably at the end)
            string[] pieces = filter.Trim().Split(Convert.ToChar(" "));

            return pieces.All((s) => folder.Directory.ToLower().Contains(s.ToLower()));
        }
        ...
        public static async Task<List<TrackViewModel>> OrderTracksAsync(IList<TrackViewModel> tracks, IList<SongViewModel> songs)
        {
            var orderedTracks = new List<TrackViewModel>();
        ...
        public static async Task<TrackViewModel> GetTrackFromSongsAsync(IList<TrackViewModel> tracks, SongViewModel song)
        {
