...
namespace Dopamine.Services.Playback
{
    public class PlaybackService : IPlaybackService
    {
        ...
        public void JumpToSecond(int timeInSeconds)
        {
            if (this.player != null && this.player.CanStop)
            {
                double totalSeconds = this.GetTotalTime.TotalSeconds;

                if (timeInSeconds < 0 && totalSeconds <= Math.Abs(timeInSeconds))
                {
                    this.player.Skip(0);
                }
                else
                {
                    this.player.Skip(Convert.ToInt32(timeInSeconds));
                }
        ...
        public async Task EnqueueFolderAsync(FolderViewModel folder)
        {
            List<string> folders = new List<string>() { folder.Directory };
            IList<Track> tracks = await this.trackRepository.GetTracksFolderAsync(folders);
            ...
            if (folder != null)
            {
                foreach (var track in tracks)
                {
                    ...
                    {
                        if (!Path.GetDirectoryName(track.Path).Contains(item))
                        {
            ...
            List<TrackViewModel> orderedTracks = await EntityUtils.OrderTracksAsync(await this.container.ResolveTrackViewModelsAsync(tracks), TrackOrder.Alphabetical);
            await this.EnqueueAsync(orderedTracks, orderedTracks.FirstOrDefault());
        }

        public async Task EnqueueSongsAsync(IList<SongViewModel> songs, SongViewModel song, bool shuffle, bool unshuffle)
        {
            if (songs == null)
            {
                return;
            ...
            List<string> pats = songs.Select(x => x.Path).ToList();
            int cont = pats.Count;
            while(cont > 0)
            {
                if (pats.Count > 500)
                {
                    List<string> patsAux = pats.GetRange(0, 500);
                    IList<Track> tracksAux = await this.trackRepository.GetTracksFolderAsync(patsAux);
                    tracks.AddRange(await EntityUtils.OrderTracksAsync(await this.container.ResolveTrackViewModelsAsync(tracksAux), songs));
                    pats.RemoveRange(0,500);
                    cont -= 500;
                }
                else
                {
                    IList<Track> tracksAux = await this.trackRepository.GetTracksFolderAsync(pats);
                    tracks.AddRange(await EntityUtils.OrderTracksAsync(await this.container.ResolveTrackViewModelsAsync(tracksAux), songs));
                    cont = 0;
                }
            }

            await this.EnqueueAsync(tracks, await EntityUtils.GetTrackFromSongsAsync(tracks, song));
        }
        ...
        public async Task<EnqueueResult> AddMusicToQueueAsync(IList<string> artists)
        {
            IList<Track> tracks = await this.trackRepository.GetArtistTracksAsync(artists);
            List<TrackViewModel> orederedTracks = await EntityUtils.OrderTracksAsync(await this.container.ResolveTrackViewModelsAsync(tracks), TrackOrder.ByAlbum);
            return await this.AddToQueueAsync(orederedTracks);
