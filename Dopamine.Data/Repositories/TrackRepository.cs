...
namespace Dopamine.Data.Repositories
{
    public class TrackRepository : ITrackRepository
    {
        ...
        private string SelectAllSongAlbumDataQuery()
        {
            return $@"SELECT t.AlbumTitle, t.AlbumArtists, t.AlbumKey, 
        ...
        public async Task<List<Track>> GetTracksFolderAsync(IList<string> paths)
        {
            var tracks = new List<Track>();
                ...
                try
                {
                    using (var conn = this.factory.GetConnection())
                    {
                        try
                        {
                            ...
                            {
                                tracks = conn.Query<Track>(this.SelectVisibleTracksQuery() + " Order by t.DateFileCreated desc");
                            }
                        }
                        catch (Exception ex)
                        {
        ...
        public async Task<IList<AlbumData>> GetFolderAlbumDataAsync(IList<string> folders, MusicType musicType)
        {
            var albumData = new List<AlbumData>();
                ...
                try
                {
                    using (var conn = this.factory.GetConnection())
                    {
                        try
                        {
                            ...
                            if (folders != null)
                            {
                                if (musicType.Equals(MusicType.All))
                                {
                                    filterQuery = $" AND ({DataUtils.CreateOrLikeClause("TrackTitle", folders, Constants.ColumnValueDelimiter)} OR {DataUtils.CreateOrLikeClause("FileName", folders, Constants.ColumnValueDelimiter)})";
                                }
                                else if (musicType.Equals(MusicType.Track))
                                {
                                    filterQuery = $" AND ({DataUtils.CreateOrLikeClause("TrackTitle", folders, Constants.ColumnValueDelimiter)})";
                                }
                                else if (musicType.Equals(MusicType.Folder))
                                {
                                    filterQuery = $" AND ({DataUtils.CreateOrLikeClause("t.Path", folders, "")})";
                                }
                            }
                            else
                            {
